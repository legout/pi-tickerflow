"""Tests for tf_cli.next_new module."""
...
class TestFindProjectRoot:
    ...
    def test_finds_project_root_with_tf_dir(self, tmp_path: Path) -> None:
        ...
        with mock.patch("tf_cli.next_new.Path.cwd", return_value=tmp_path):
            result = next_new.find_project_root()
    ...
    def test_finds_project_root_in_parent(self, tmp_path: Path) -> None:
        ...
        with mock.patch("tf_cli.next_new.Path.cwd", return_value=subdir):
            result = next_new.find_project_root()
            assert result == tmp_path
...
class TestLoadTicketQuery:
    ...
    def test_loads_query_from_config(self, tmp_path: Path) -> None:
        ...
        result = next_new.load_ticket_query(config_file)
...
class TestResolveQuery:
    ...
    def test_uses_config_query_when_available(self, tmp_path: Path) -> None:
        ...
        result = next_new.resolve_query(tmp_path)
    ...
    def test_uses_default_when_no_config(self, tmp_path: Path) -> None:
        ...
        result = next_new.resolve_query(None)
        assert result == next_new.DEFAULT_QUERY
...
class TestRunQuery:
    ...
    def test_runs_query_and_returns_first_word(self) -> None:
        ...
        with mock.patch("subprocess.run", return_value=mock_result):
            result = next_new.run_query("tk ready")
...
class TestMain:
    ...
    def test_error_when_no_tickets_found(self) -> None:
        ...
        with mock.patch("shutil.which", return_value="/usr/bin/tk"):
            with mock.patch.object(next_new, "run_query", return_value=None):
                result = next_new.main([])
                assert result == 1
    ...
    def test_success_prints_ticket(self, capsys) -> None:
        ...
        with mock.patch("shutil.which", return_value="/usr/bin/tk"):
            with mock.patch.object(next_new, "run_query", return_value="abc-123"):
                result = next_new.main([])
    ...
    def test_with_project_arg(self, tmp_path: Path) -> None:
        ...
        with mock.patch("shutil.which", return_value="/usr/bin/tk"):
            with mock.patch.object(next_new, "run_query", return_value="xyz-789"):
                result = next_new.main(["--project", str(tmp_path)])
