...
get_git_tag_version() {
  ...
  if [ -n "$tag" ]; then
    # Normalize tag (strip v prefix)
    echo "$(normalize_version "$tag")"
  fi
}
...
check_version_consistency() {
  ...
  # Get versions from all manifests
  ...
  # Build list of found manifests
  local found_manifests=()
  local all_versions=()
  local canonical_version=""
  local canonical_manifest=""

  # Check pyproject.toml (highest priority)
  if [ -f "$pyproject_file" ]; then
    found_manifests+=("pyproject.toml")
    if [ -n "$pyproject_version" ]; then
      all_versions+=("pyproject.toml:$pyproject_version")
      if [ -z "$canonical_version" ]; then
        canonical_version="$pyproject_version"
        canonical_manifest="pyproject.toml"
      fi
    else
      all_versions+=("pyproject.toml:invalid")
    fi
  fi

  # Check Cargo.toml (second priority)
  if [ -f "$cargo_file" ]; then
    found_manifests+=("Cargo.toml")
    if [ -n "$cargo_version" ]; then
      all_versions+=("Cargo.toml:$cargo_version")
      if [ -z "$canonical_version" ]; then
        canonical_version="$cargo_version"
        canonical_manifest="Cargo.toml"
      fi
    else
      all_versions+=("Cargo.toml:invalid")
    fi
  fi

  # Check package.json (third priority)
  if [ -f "$package_file" ]; then
    found_manifests+=("package.json")
    if [ -n "$package_version" ]; then
      all_versions+=("package.json:$package_version")
      if [ -z "$canonical_version" ]; then
        canonical_version="$package_version"
        canonical_manifest="package.json"
      fi
    else
      all_versions+=("package.json:invalid")
    fi
  fi
  ...
  for v in "${all_versions[@]}"; do
    local manifest_name="${v%%:*}"
    local manifest_version="${v#*:}"
    if [ "$manifest_version" = "invalid" ]; then
      echo "[info] $manifest_name: version field missing or invalid"
    else
      echo "[ok] $manifest_name version: $manifest_version"
    fi
  done

  # Check for version mismatches between manifests
  if [ "${#found_manifests[@]}" -gt 1 ]; then
    ...
    for v in "${all_versions[@]}"; do
      local manifest_name="${v%%:*}"
      local manifest_version="${v#*:}"
      # Skip the canonical manifest in mismatch check
      if [ "$manifest_name" = "$canonical_manifest" ]; then
        continue
      fi
      if [ "$manifest_version" != "invalid" ]; then
        local normalized_version
        normalized_version=$(normalize_version "$manifest_version")
        if [ "$normalized_version" != "$normalized_canonical" ]; then
          mismatches+=("$manifest_name=$manifest_version")
        fi
      fi
    done
    ...
    if [ "${#mismatches[@]}" -gt 0 ]; then
      ...
      echo "       Canonical (first valid): $canonical_manifest = $canonical_version"
      for m in "${mismatches[@]}"; do
        echo "       Mismatch: ${m%%=*} = ${m#*=}"
      done
      echo "       Consider aligning versions across all manifests"
      # Don't fail for manifest mismatches, just warn
    fi
  fi
  ...
  if [ -n "$git_tag_version" ]; then
    ...
    if [ "$git_tag_version" != "$normalized_canonical" ]; then
      echo "[warn] Git tag ($git_tag_version) does not match $canonical_manifest ($canonical_version)"
      echo "       This may indicate a version mismatch in the release"
      # Don't fail for git tag mismatch, just warn
    else
  ...
  if [ -n "$version_file_version" ]; then
    ...
    if [ "$normalized_file" != "$normalized_canonical" ]; then
      if [ "$dry_run" = "true" ]; then
        ...
        echo "[warn] VERSION file ($version_file_version) does not match $canonical_manifest ($canonical_version)"
        echo "       To fix: run 'tf doctor --fix' or update VERSION file manually"
        return 1
      fi
    else
      echo "[ok] VERSION file matches $canonical_manifest: $version_file_version"
      return 0
    fi
  else
